...
// 管理画面用: お問い合わせ一覧取得
async function _getContactsAction(limit = 20, offset = 0, isRead?: boolean): Promise<ActionState> {
  try {
...
// 管理画面用: お問い合わせ詳細取得
async function _getContactAction(id: number): Promise<ActionState> {
  try {
...
// 管理画面用: 既読にする
async function _markContactAsReadAction(id: number): Promise<ActionState> {
  try {
