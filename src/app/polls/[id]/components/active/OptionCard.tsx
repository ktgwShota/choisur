      ...
      <div className="flex min-h-0 flex-grow flex-col p-6">
        {option.title &&
          (option.url ? (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="mb-4 line-clamp-2 cursor-pointer overflow-hidden text-ellipsis font-bold text-lg text-slate-800 leading-snug no-underline transition-colors hover:text-blue-600 hover:underline"
            >
              {option.title}
            </a>
          ) : (
            <div
              id={`option-title-${option.id}`}
              className="mb-4 line-clamp-2 overflow-hidden text-ellipsis font-bold text-base text-slate-800 leading-snug"
            >
              {option.title}
            </div>
          ))}
        ...
        <div className="mt-auto flex flex-col border-slate-100 border-t pt-4">
          ...
          <Button
            ...
            className={cn(
              'group/btn relative h-auto w-full rounded-[2px] px-5 py-4 transition-all duration-300',
              isVoted
                ? 'cursor-default border-2 border-blue-200 bg-white text-blue-600 hover:border-blue-200 hover:bg-blue-50 hover:text-blue-600'
                : 'border-2 border-[#1976d2] bg-[#1976d2] text-white shadow-md hover:border-[#1565c0] hover:bg-[#1565c0] active:scale-[0.98]'
            )}
          >
            <div
              className={cn(
            ...
            >
              {isVoted ? (
                ...
                <ThumbsUp size={14} strokeWidth={2} className="text-white" />
              )}
              <span className="text-sm">{isVoted ? '投票済み' : '投票する'}</span>
            </div>
...
export function VoterList({ voters }: VoterListProps) {
  return (
    <div className="mb-6">
      <span className="mb-2 block text-slate-500 text-xs">投票者</span>
      {voters && voters.length > 0 ? (
        <div className="flex flex-wrap items-center gap-2">
          {voters.slice(0, 5).map((voter) => (
            <div
              key={voter.id}
              className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-white font-bold text-white text-xs ring-1 ring-slate-100"
              style={{
        ...
        <div className="flex h-8 items-center text-slate-400 text-xs">まだ投票はありません</div>
      )}
    </div>
  );
}
    ...
    <>
      <div className="mb-3 flex items-end justify-between">
        <div className="flex items-end gap-1.5">
          <span className="font-black text-slate-800 text-xl leading-none">{votes}</span>
          <span className="text-slate-500 text-xs">票</span>
        </div>
        <span className="font-bold text-slate-600 text-sm leading-none">
