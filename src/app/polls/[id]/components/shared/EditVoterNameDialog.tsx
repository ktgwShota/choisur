...
interface EditVoterNameDialogProps {
  open: boolean;
...
export default function EditVoterNameDialog({
  open,
  onOpenChange,
  initialVoterName,
  onSubmit,
}: EditVoterNameDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      reset({ voterName: initialVoterName || '' });
    }
  }, [initialVoterName, open, reset]);
    ...
    <FadeDialog
      ...
      header={
        <DialogHeader className="sr-only">
          <DialogTitle>投票者名の変更</DialogTitle>
        </DialogHeader>
      }
      contents={
