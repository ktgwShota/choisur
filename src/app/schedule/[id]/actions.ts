...
import { ORGANIZER_UNAUTHORIZED_ERROR, verifyScheduleOrganizerToken } from '@/lib/organizer-auth';
...
// スケジュール取得
async function _getScheduleAction(id: string): Promise<ActionState> {
  try {
...
export async function deleteScheduleAction(
  ...
  try {
    if (!(await verifyScheduleOrganizerToken(id, organizerToken))) {
...
// 回答の削除
async function _deleteResponseAction(
  scheduleId: string,
  respondentId: string
): Promise<ActionState> {
  try {
