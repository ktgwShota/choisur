      ...
      {hasNameColumn && (
        <div className="relative sticky left-0 z-20 w-[150px] min-w-[150px] max-w-[150px] shrink-0 border-slate-200 border-r bg-white">
          ...
          <div className="absolute bottom-2 left-2 flex items-center gap-1.5 text-slate-500">
            <span className="font-medium text-[11px] leading-none">参加者</span>
          </div>
        </div>
      )}
      ...
      >
        {showMyBadge ? (
          <button
            ...
            disabled={!isMyResponse || !onEdit}
            className={`flex max-w-full items-center border-0 bg-transparent p-0 text-left ${isMyResponse && onEdit ? 'cursor-pointer hover:opacity-70' : 'cursor-default'}`}
          >
    ...
    <div className="-mt-[1px] flex w-max min-w-full border-blue-500 border-t border-b">
      ...
      <div className="sticky left-0 z-10 flex w-[150px] min-w-[150px] max-w-[150px] shrink-0 items-center justify-center border-slate-200 border-r bg-blue-50 p-5">
        <Input
          ...
          className="!text-[14px] h-auto w-full border-none bg-transparent px-0 py-0 text-center shadow-none outline-none ring-0 placeholder:text-slate-400 focus-visible:ring-0 focus-visible:ring-offset-0"
        />
      </div>
