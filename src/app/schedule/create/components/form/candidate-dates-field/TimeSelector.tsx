...
import { Slider } from '@/components/shared/primitives/slider';
...
export default function TimeSelector(_props: TimeSelectorProps) {
  const { selectedDates, lastSelectedDates, selectPrevDate, selectNextDate, canGoPrev, canGoNext } =
    useCandidateDates();
  ...
  if (isDisabled) {
    return (
      <Tooltip>
        ...
        <TooltipContent className="rounded-[2px] bg-gray-800 p-3 text-white [&_svg]:fill-gray-800">
          日付を選択すると利用可能になります
        </TooltipContent>
      </Tooltip>
    );
    ...
    <div className="flex justify-between px-1 font-medium text-slate-400">
      {QUICK_TIME_PRESETS.map((preset) => (
        <button
          ...
          className="cursor-pointer border-0 bg-transparent p-0 font-medium text-slate-400 transition-colors hover:text-slate-600"
          style={{ fontSize: getResponsiveValue(10, 12) }}
        >
...
function TimeSliderControl({ hour, minute, onTimeChange }: TimeSliderControlProps) {
