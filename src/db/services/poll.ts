...
import { createOrganizerToken } from '@/lib/organizer-auth';
...
}): Promise<DBResult<{ id: string; organizerToken: string }>> {
  try {
    const db = getDrizzle();
    ...
    return { success: true, data: { id: pollId, organizerToken } };
  } catch (error) {
...
export async function getPoll(pollId: string): Promise<DBResult<ParsedPoll>> {
  ...
  const { organizerToken: _organizerToken, ...publicPoll } = poll;
  const pollData = {
    ...publicPoll,
...
async function _getPolls(): Promise<DBResult<ParsedPoll[]>> {
  const db = getDrizzle();
...
export async function reopenPoll(pollId: string): Promise<DBResult<{ id: string }>> {
  const db = getDrizzle();
  await db.update(polls).set({ isClosed: false }).where(eq(polls.id, pollId));
  return { success: true, data: { id: pollId } };
}
...
export async function votePoll(data: {
  ...
  const targetVoters = JSON.parse(targetOption.voters || '[]') as Array<{
    id: string;
    name: string;
  }>;
  const existingVoterIndex = targetVoters.findIndex((v) => v.id === data.voterId);

  // 既に投票済みの場合は取り消し（他の選択肢からも削除）
  if (existingVoterIndex !== -1) {
    // 対象の選択肢から削除
    targetVoters.splice(existingVoterIndex, 1);
    await db
      .update(pollOptions)
      .set({
        votes: Math.max(0, (targetOption.votes || 0) - 1),
        voters: JSON.stringify(targetVoters),
      })
    ...
    for (const opt of allOptions) {
      if (opt.optionId !== data.optionId) {
        ...
        if (voterIndex !== -1) {
          ...
          await db
            .update(pollOptions)
            .set({ votes: Math.max(0, (opt.votes || 0) - 1), voters: JSON.stringify(voters) })
            .where(
              and(eq(pollOptions.pollId, data.pollId), eq(pollOptions.optionId, opt.optionId))
            );
        }
      }
    }
    ...
    // 対象の選択肢に追加
    targetVoters.push({ id: data.voterId, name: data.voterName });
    await db
      .update(pollOptions)
      .set({ votes: (targetOption.votes || 0) + 1, voters: JSON.stringify(targetVoters) })
      .where(and(eq(pollOptions.pollId, data.pollId), eq(pollOptions.optionId, data.optionId)));
  }
