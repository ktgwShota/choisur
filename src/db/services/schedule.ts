...
export async function createSchedule(data: {
  ...
  try {
    ...
    return { success: true, data: { id: scheduleId, organizerToken } };
  } catch (error) {
...
async function _getSchedules(): Promise<DBResult<Schedule[]>> {
  try {
    const db = getDrizzle();
...
export async function updateResponse(data: {
  ...
  try {
    ...
    const updateData: { name?: string; availability?: string } = {};
